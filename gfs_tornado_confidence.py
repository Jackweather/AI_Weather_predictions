...
TORNADO_DOWNLOAD_FORECAST_HOURS = (0,) + tuple(range(6, MAX_DOWNLOAD_FORECAST_HOUR + 1, 6))
...
FIELD_SPECS = {
    ...
    "cin": {
        "backend_kwargs": (
            {"filter_by_keys": {"shortName": "cin", "typeOfLevel": "pressureFromGroundLayer"}, "indexpath": ""},
            {"filter_by_keys": {"shortName": "cin", "typeOfLevel": "heightAboveGroundLayer"}, "indexpath": ""},
            {"filter_by_keys": {"shortName": "cin"}, "indexpath": ""},
        ),
        "selectors": (
            ("cin",),
            ("CIN",),
    ...
    "hlcy": {
        "backend_kwargs": (
            {"filter_by_keys": {"shortName": "hlcy", "typeOfLevel": "heightAboveGroundLayer"}, "indexpath": ""},
            {"filter_by_keys": {"shortName": "hlcy"}, "indexpath": ""},
        ),
        "selectors": (
            ("hlcy",),
            ("HLCY",),
            ("GRIB_shortName", "hlcy"),
            ("GRIB_name", "Storm relative helicity"),
        ),
    },
    "refc": {
        "backend_kwargs": (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "and build 0-10 tornado confidence maps from aligned severe-weather ingredients."
        )
    )
    parser.add_argument(
        "--output-root",
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="How many cycle folders to keep in tornado_grib and plots before deleting older ones.",
    )
    parser.add_argument(
        "--cycle-lookback",
        type=int,
...
def build_tornado_url(run_cycle: RunCycle, forecast_hour: int) -> str:
    return (
...
def local_grib_path(root: Path, run_cycle: RunCycle, forecast_hour: int) -> Path:
    file_name = f"tornado_{run_cycle.file_name(forecast_hour)}.grib2"
    return root / "tornado_grib" / run_cycle.tag / file_name
...
def probe_file_available(client: NomadsClient, run_cycle: RunCycle, forecast_hour: int) -> bool:
    url = build_tornado_url(run_cycle, forecast_hour)
    try:
...
def resolve_latest_complete_cycle(now_utc: dt.datetime, lookback_cycles: int, client: NomadsClient) -> RunCycle:
    ...
    for offset in range(lookback_cycles + 1):
        ...
        if probe_file_available(client, run_cycle, MAX_DOWNLOAD_FORECAST_HOUR):
            logging.info("Selected latest complete tornado run: %s", run_cycle.tag)
            return run_cycle
        logging.info("Tornado run %s is not complete yet, falling back one cycle", run_cycle.tag)
    raise RuntimeError("Could not find a complete recent GFS tornado run within the configured lookback window.")
...
def ensure_complete_history(run_cycles: Sequence[RunCycle], client: NomadsClient) -> None:
    for run_cycle in run_cycles:
        if not probe_file_available(client, run_cycle, MAX_DOWNLOAD_FORECAST_HOUR):
            raise RuntimeError(
                f"Run {run_cycle.tag} does not appear complete for tornado fields on NOMADS. "
...
def download_run(
    ...
    if not hours_to_download:
        logging.info("Skipping %s because all %d tornado files already exist", run_cycle.tag, len(forecast_hours))
        return

    logging.info(
        "Downloading tornado fields for %s (%d missing of %d files)",
        run_cycle.tag,
...
def load_tornado_fields(grib_path: Path) -> tuple[dict[str, np.ndarray], np.ndarray, np.ndarray]:
    ...
    if lats is None or lons is None:
        raise RuntimeError(f"Could not read tornado fields from {grib_path.name}.")

    return fields, lats, lons


def calculate_member_tornado_score(fields: dict[str, np.ndarray]) -> np.ndarray:
    cape = np.maximum(fields["cape"], 0.0)
    cin = np.minimum(fields["cin"], 0.0)
    helicity = np.maximum(fields["hlcy"], 0.0)
    reflectivity = np.maximum(fields["refc"], 0.0)

    cape_score = np.clip(cape / 2100.0, 0.0, 1.0)
    cin_support = np.clip(1.0 - (np.abs(cin) / 225.0), 0.0, 1.0)
    helicity_score = np.clip(helicity / 280.0, 0.0, 1.0)
    reflectivity_score = np.clip((reflectivity - 8.0) / 37.0, 0.0, 1.0)
    ...
    suppress_nonconvective = np.where(
        (cape < 175.0) | (reflectivity < 8.0),
        0.0,
        1.0,
    )
    member_score = np.clip(10.0 * raw_score * suppress_nonconvective, 0.0, 10.0)
...
def collect_aligned_members(
    ...
    for offset, run_cycle in enumerate(run_cycles):
        ...
        if not grib_path.exists():
            logging.warning("Missing aligned tornado member for %s at f%03d", run_cycle.tag, aligned_hour)
            continue

        try:
            fields, current_lats, current_lons = load_tornado_fields(grib_path)
        except Exception as exc:
            logging.warning("Skipping invalid tornado member %s at f%03d: %s", run_cycle.tag, aligned_hour, exc)
            continue
        ...
        member_arrays.append(calculate_member_tornado_score(fields))
        metadata.append((run_cycle.tag, aligned_hour))

    if not member_arrays or lats is None or lons is None:
        raise RuntimeError(f"No aligned tornado members were available for forecast hour f{current_forecast_hour:03d}.")
...
def calculate_tornado_confidence(member_scores: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    active_members = member_scores >= 3.8
    ...
    confidence = np.where(mean_score >= 0.9, confidence, 0.0)
...
def tornado_cmap() -> tuple[ListedColormap, BoundaryNorm]:
    boundaries = np.arange(0, 11, 1)
    cmap = ListedColormap(TORNADO_COLORS, name="tornado_confidence")
...
def build_plot_title(run_cycle: RunCycle, forecast_hour: int) -> str:
    ...
    return (
        f"Tornado Confidence Forecast F{forecast_hour:03d}\n"
        f"Valid {day_of_week}, {valid_local.strftime('%b %d, %Y')} at {hour_string} ET"
    )


def plot_tornado_map(
    save_path: Path,
    run_cycle: RunCycle,
    forecast_hour: int,
    lats: np.ndarray,
    ...
    projected_confidence = np.ma.masked_where(smoothed_mean_score < 0.9, smoothed_confidence)
    ...
    colorbar.set_label("Tornado confidence (0-10)")
...
def build_tornado_products(
    ...
    for forecast_hour in plot_forecast_hours:
        ...
        confidence, mean_score, _ = calculate_tornado_confidence(members)
        save_path = plot_root / f"tornado_confidence_f{forecast_hour:03d}.png"
        plot_tornado_map(
            save_path=save_path,
            run_cycle=run_cycle,
        ...
        logging.info("Saved %s using %d aligned tornado members", save_path, len(metadata))
...
def main() -> None:
    ...
    prune_old_run_directories(output_root / "tornado_grib", args.retain_runs)
    ...
    if not args.skip_download:
        for run_cycle in run_cycles:
            download_run(
                ...
                root=output_root,
                run_cycle=run_cycle,
                forecast_hours=TORNADO_DOWNLOAD_FORECAST_HOURS,
                overwrite=args.overwrite,
                workers=args.download_workers,
            )
        prune_old_run_directories(output_root / "tornado_grib", args.retain_runs)

    build_tornado_products(
        root=output_root,
    ...
    logging.info("Finished building tornado confidence maps for %s", current_run.tag)
